...
- One of the core design principles of many NoSQL DBMSs has been **horizontal scaling**, which distributes data storage and computation to multiple machines in the system
...
- **Horizontal scalability** offers performance and fault tolerance, but often trades away **strong consistency**. Written data is only _"eventually"_ propagated across all machines in the system and until then, users may read **stale, outdated data**
- **Schema flexibility** makes it easy to evolve the data model, but often leads to **low data quality**, since no strict rules are enforced on the structure of the data (though enforcement is possible in some NoSQL DBMSs, such as MongoDB)
